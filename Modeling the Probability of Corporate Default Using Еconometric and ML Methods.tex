...
\begin{table}[H]
\centering
\caption{Polish macroeconomic context, 2000--2013.$^{*}$}
...
\caption*{\footnotesize $^{*}$ GDP growth and CPI inflation are annual
...
\begin{table}[ht]
\centering
\caption{Sample sizes and class balance by prediction horizon.$^{*}$}
\label{tab:horizons}
\centering
\resizebox{\textwidth}{!}{%
...
\caption*{\footnotesize $^{*}$ The final two columns show the bankruptcy rate
...
\begin{table}[ht]
\centering
\caption{Altman ratio means by bankruptcy status at H1.$^{*}$}
...
\caption*{\footnotesize $^{*}$ Each ratio is winsorized at the 1st and 99th percentiles within its group. Welch's $t$-test allows for unequal group variances. Cohen's $d$ is the standardized mean difference ($|d|>0.2$ small, $>0.5$ medium, $>0.8$ large). Source: author's calculations.}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Re-estimated econometric models at horizon H1.$^{*}$}
\label{tab:econ-coef}
\small
\begin{tabular}{lrrrr}
\toprule
...
\bottomrule
\end{tabular}
\caption*{\footnotesize $^{*}$ Continuous predictors are standardized, so each
...
\begin{table}[ht]
\centering
\caption{Performance of the econometric baselines at horizon H1.$^{*}$}
\label{tab:econ-cv}
\small
...
\bottomrule
\end{tabular}
\caption*{\footnotesize $^{*}$ Means across five folds. AUC and PR-AUC measure ranking ability; Brier and ECE measure calibration (lower is better); recall and precision are at the F1-optimal threshold; normalized cost is relative to the naive ``predict everyone healthy'' rule (below one is better). Base rate 6.94\%. Source: author's calculations.}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Top variables selected by the LASSO logit, H1.$^{*}$}
...
\bottomrule
\end{tabular}
\caption*{\footnotesize $^{*}$ Penalty $C = 0.1$. Coefficients are on
...
\bottomrule
\end{tabular}
\caption*{\footnotesize Means across five folds. Recall and precision are
...
features. Source: author's calculations.}
\end{table}
...
The numbers above are internally consistent, but it is worth asking how they
compare with what other authors have reported on the same Polish dataset. This
matters for two reasons. First, it checks that our pipeline is sound: if our
XGBoost were far below published results, that would point to an implementation
problem. Second, it shows where our contribution sits relative to the existing
literature. Table~\ref{tab:litbenchmark} collects the main published results at
the one-year-ahead horizon, and Figure~\ref{fig:litbenchmark} plots them
against ours.
...
\cite{smiti2020}   & Deep AE + Borderline-SMOTE & $>$0.95 & oversampling \\
\midrule
...
\bottomrule
\end{tabular}
\caption*{\footnotesize $^{*}$ AUC is the metric reported across this
...
Three points follow from the comparison. First, our XGBoost AUC of 0.960 is at
the top of the published range, marginally above the 0.955 that
\cite{zieba2016} report for their best model (an ensemble of boosted trees with
evolved synthetic features) and above their plain XGBoost at 0.951. This is
reassuring: our pipeline reaches the same ceiling that specialized studies
reach, so the comparison with the classical models is not flattered by a weak
ML implementation. Second, our Ohlson baseline at 0.795 sits squarely in the
classical-model band, close to the LDA result of \cite{zieba2016} at 0.796 and
their cost-sensitive logit at 0.821, and well above their plain logistic
regression at 0.632. The classical models behave here as they do elsewhere in
the literature. Third, and most important for the contribution of this paper,
the published studies report ROC-AUC almost exclusively. None of them report
PR-AUC, calibration, or a like-for-like comparison against re-estimated
classical baselines under one protocol. Our value added is therefore not a
higher AUC, the AUC ceiling on this dataset is already reached, but the
broader and more honest evaluation: the same protocol for every model, the
rare-event metric that matters under heavy imbalance, calibration before and
after recalibration, and an explicit decomposition of where the advantage
comes from.
...
\bottomrule
\end{tabular}
\caption*{\footnotesize Source: author's calculations.}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Performance across prediction horizons.$^{*}$}
...
\bottomrule
\end{tabular}
\caption*{\footnotesize $^{*}$ Means across five folds. H1 is one year ahead, H5 is five years ahead. Upper panel: ROC-AUC; lower panel: PR-AUC. Source: author's calculations.}
\end{table}
...
\begin{table}[H]
\centering
\caption{Performance by model class and feature set at H1.$^{*}$}
...
\bottomrule
\end{tabular}
\caption*{\footnotesize $^{*}$ LASSO and Ridge are omitted on the Altman-5 and
...
Altman/Ohlson specifications. Source: author's calculations.}
\end{table}
...
project repository:\\[4pt]
\url{https://github.com/ilyas1python/Modeling-the-Probability-of-Corporate-Default-Using-Econometric-and-Machine-Learning-Methods}
}}
\end{center}
